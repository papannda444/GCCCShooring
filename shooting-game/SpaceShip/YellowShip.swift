...
extension YellowShip: SpaceShip {
    ...
    func touchViewBegin(touchedViewFrame frame: CGRect) {
        ...
        let bullet = Bullet(bulletType: .yellow, bulletLevel: level, position: position)
        ...
        bulletTimer = Timer.scheduledTimer(withTimeInterval: 0.1, repeats: true) { [weak self, level] _ in
            let bullet = Bullet(bulletType: .yellow, bulletLevel: level, position: self?.position ?? .zero)
            bullet.run(moveToTop)
